...
class PhysicsInformedLoss(nn.Module):
    ...
    def __init__(self, labmda_con, dx):
    ...
    def forward(self, gen_output, HR_image):
        ...
        strain_rate_2_HR = torch.mean(torch.mean(self.get_strain_rate_mags(
            vel_grad_HR), axis=1, keepdim=True), axis=2, keepdim=True)
    ...
    def ddx(self, input, channel):
        return self.first_order_diff_x(input[:, channel, :, :]) / self.dx

    def ddy(self, input, channel):
        return self.first_order_diff_y(input[:, channel, :, :]) / self.dx

    def d2dx2(self, input, channel):
        return self.second_order_diff_x(input[:, channel, :, :]) / self.dx ** 2

    def d2dy2(self, input, channel):
        return self.second_order_diff_y(input[:, channel, :, :]) / self.dx ** 2
