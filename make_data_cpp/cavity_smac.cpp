...
#define ni 128 // 格子分割数（x方向）
#define nj 128 // 格子分割数（y方向）

double u0 = 10.0,   // 上壁の速度
    ...
    an[ni + 2][nj + 2], as[ni + 2][nj + 2],
       ...
       pre_u[ni + 2][ni + 2], pre_v[ni + 2][ni + 2], // 保存用の前回の速度
    dx, dy,                                          // 格子サイズ
    visc,                                            // 動粘性係数
    dvmax,                                           // 速度vの変化量の最大値
    pcnew, err,
...
void PlotVector(), PlotStreamline(), PlotPresure(), SaveData(), SaveData_per_time_p(double), SaveData_per_time_u(double), SaveData_per_time_v(double); // 結果の表示ルーチン等
...
{
  //***** gnuplot と接続 *****
  fp = popen("C:/gnuplot/bin/gnuplot", "w");
  ...
  do
  {
    ...
    for (i = 1; i <= ni; i++)
    {
      for (j = 1; j <= nj; j++)
      {
        ...
        //if(j != nj)
        v[i][j] = vnew[i][j] + (pc[i][j] - pc[i][j + 1]) / dy * dt;
        p[i][j] = p[i][j] + pc[i][j];
        dvmax = MAX(dvmax, fabs(v[i][j] - vold));
      }
    }
    printf("time_step = %.3lf ", time_step);
    ...
    for (i = 1; i <= ni; i++)
    {
      u[i][0] = -u[i][1];               // 下境界 u=0
      u[i][nj + 1] = 2 * u0 - u[i][nj]; // 上境界 u=u0
                                        //p[i][0] = p[i][1]; p[i][nj + 1] = p[i][nj];
    }
    for (j = 1; j <= nj; j++)
    {
      v[0][j] = -v[1][j];       // 左境界 v=0
      v[ni + 1][j] = -v[ni][j]; // 右境界 v=0
                                //p[0][j] = p[1][j]; p[ni + 1][j] = p[ni][j];
    }

    if (n < 100)
    {
      SaveData_per_time_p(time_step);
    }
    ...
    memcpy(pre_u, u, sizeof(u));
    memcpy(pre_v, u, sizeof(v));

    //SaveData_per_time_u(time_step);
    //SaveData_per_time_v(time_step);

    //***** 途中結果の表示 *****
    printf("n=%d iter=%d dvmax=%f\n", n, iter, dvmax);

    if (n % 200 == 0)
      PlotVector(); // 計算途中の速度ベクトル図を描く（計算の進行状況を見たいとき）
...
void sor_pc()
{
  ...
  do
  {
    ...
    for (i = 1; i <= ni; i++)
    {
      for (j = 1; j <= nj; j++)
      {
        pcnew = (aw[i][j] * pc[i - 1][j] + ae[i][j] * pc[i + 1][j] + as[i][j] * pc[i][j - 1] + an[i][j] * pc[i][j + 1] + b[i][j]) / ap[i][j];
...
void SaveData_per_time_p(double _time)
{
  FILE *fpp;
  FILE *fpu;
  FILE *fpv;
  ...
  char filename_u[100];
  char filename_v[100];
  sprintf(filename_p, "C:\\Users\\exm_c\\OneDrive\\ドキュメント\\fluid_csv_files\\128_128_u0=10_51step_Re100\\%.6lf_p.csv", _time);
  sprintf(filename_u, "C:\\Users\\exm_c\\OneDrive\\ドキュメント\\fluid_csv_files\\128_128_u0=10_51step_Re100\\%.6lf_u.csv", _time);
  sprintf(filename_v, "C:\\Users\\exm_c\\OneDrive\\ドキュメント\\fluid_csv_files\\128_128_u0=10_51step_Re100\\%.6lf_v.csv", _time);
  fpp = fopen(filename_p, "w");
  fpu = fopen(filename_u, "w");
  fpv = fopen(filename_v, "w");
  ...
  for (int j = 0; j <= nj + 1; j++)
  {
    ...
    for (int i = 0; i <= ni + 1; i++)
    {
      if (i == ni + 1)
      {
        line_p = line_p + std::to_string(p[i][j]) + "\n";
        line_u = line_u + std::to_string(u[i][j]) + "\n";
        line_v = line_v + std::to_string(v[i][j]) + "\n";
      }
      else
      {
        line_p = line_p + std::to_string(p[i][j]) + ",";
    ...
    fprintf(fpp, line_p.c_str());
    fprintf(fpu, line_u.c_str());
    fprintf(fpv, line_v.c_str());
  }
  fclose(fpp);
  fclose(fpu);
  fclose(fpv);
...
void SaveData_per_time_u(double _time)
{
  FILE *fp4;
  char filename[100];
  sprintf(filename, "C:\\Users\\exm_c\\OneDrive\\ドキュメント\\fluid_csv_files\\test\\%.3lf_u.csv", _time);
  ...
  for (int j = 1; j <= nj; j++)
  {
    ...
    fprintf(fp4, line.c_str());
  }
  fclose(fp4);
}

void SaveData_per_time_v(double _time)
{
  FILE *fp4;
  char filename[100];
  sprintf(filename, "C:\\Users\\exm_c\\OneDrive\\ドキュメント\\fluid_csv_files\\test\\%.3lf_v.csv", _time);
  fp4 = fopen(filename, "w");
  for (int j = 1; j <= nj; j++)
  {
    std::string line = "";
