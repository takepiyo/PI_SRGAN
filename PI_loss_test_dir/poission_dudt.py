...
for i in range(ni + 1):
    for j in range(nj + 1):
        ...
        flux_e_for[:, :, j, i] = ue_for[:, :, j, i] * (u_array[:, :, j, i + 1] + u_array[:, :, j, i]) / 2 - \
            visc_per_dx * (u_array[:, :, j, i + 1] - u_array[:, :, j, i])

        flux_n_for[:, :, j, i] = vn_for[:, :, j, i] * (u_array[:, :, j + 1, i] + u_array[:, :, j, i]) / 2 - \
            visc_per_dx * (u_array[:, :, j + 1, i] - u_array[:, :, j, i])

dudt_max_for = 0.0

e_diff_for = torch.zeros(1, 1, nj + 1, ni)
n_diff_for = torch.zeros(1, 1, nj + 1, ni)
p_diff_for = torch.zeros(1, 1, nj + 1, ni)

for i in range(1, ni):
    for j in range(1, nj + 1):
        a = -(-flux_e_for[:, :, j, i - 1] + flux_e_for[:, :, j, i]) / dx
        b = -(-flux_n_for[:, :, j - 1, i] + flux_n_for[:, :, j, i]) / dx
        c = -(-p_array[:, :, j, i] + p_array[:, :, j, i + 1]) / dx
        e_diff_for[:, :, j, i] = a
        n_diff_for[:, :, j, i] = b
        p_diff_for[:, :, j, i] = c

        dudt = a + b + c
        if dudt > dudt_max_for:
            dudt_max_for = dudt
...
ue_conv = ave_x(u_array)
vn_conv = ave_x(v_array)

ue_diff = compare_tensor(ue_conv, ue_for)
vn_diff = compare_tensor(vn_conv, vn_for)

flux_e_conv = ue_conv * ave_x(u_array) - visc_per_dx * ddx_up(u_array)
flux_n_conv = vn_conv * ave_y(u_array) - visc_per_dx * ddy_up(u_array)
